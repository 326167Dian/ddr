...
@section('content')
    ...
    <div class="mb-2">
        <input type="text" id="search-articles" class="form-control table-search" placeholder="Cari judul atau slug...">
    </div>

    <div class="card"><div class="table-responsive"><table class="table">
        <thead><tr><th>Drag</th><th>Urut</th><th>Judul</th><th>Slug</th><th>Tayang</th><th>Status</th><th>Aksi</th></tr></thead>
        ...
        @forelse($items as $item)
            <tr data-id="{{ $item->id }}">
                ...
                <td>
                    <a href="{{ route('admin.articles.edit', $item) }}" class="btn btn-secondary btn-sm">Edit</a>
                    <form action="{{ route('admin.articles.destroy', $item) }}" method="POST" class="d-inline">@csrf @method('DELETE')<button class="btn btn-danger btn-sm" onclick="return confirm('Hapus data?')">Hapus</button></form>
                </td>
            </tr>
        @empty
...
<script>
    (function () {
        const tbody = document.getElementById('sortable-articles');
        ...
        const persistOrder = async () => {
            ...
            const response = await fetch('{{ route('admin.articles.reorder') }}', {
                method: 'POST',
                headers: {
        ...
        tbody.querySelectorAll('tr[data-id]').forEach((row) => {
            ...
            handle.addEventListener('dragend', async () => {
                ...
                if (isDirty) {
                    await persistOrder();
                    if (window.showAdminToast) window.showAdminToast('Urutan artikel tersimpan');
                }
            });
