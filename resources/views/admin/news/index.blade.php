...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <div>
            <a href="{{ route('admin.news.create') }}" class="btn btn-primary btn-sm">Tambah</a>
        </div>
    </div>

    <div class="mb-2">
        <input type="text" id="search-news" class="form-control table-search" placeholder="Cari judul atau slug...">
    </div>
        ...
        @forelse($items as $item)
            <tr data-id="{{ $item->id }}">
                ...
                <td>
                    ...
                    <form action="{{ route('admin.news.destroy', $item) }}" method="POST" class="d-inline">@csrf @method('DELETE')<button class="btn btn-danger btn-sm" onclick="return confirm('Hapus data?')">Hapus</button></form>
                </td>
            </tr>
        @empty
...
<script>
    (function () {
        const tbody = document.getElementById('sortable-news');
        ...
        const persistOrder = async () => {
            ...
            const response = await fetch('{{ route('admin.news.reorder') }}', {
                method: 'POST',
                headers: {
        ...
        tbody.querySelectorAll('tr[data-id]').forEach((row) => {
            ...
            handle.addEventListener('dragend', async () => {
                ...
                if (isDirty) {
                    await persistOrder();
                    if (window.showAdminToast) window.showAdminToast('Urutan berita tersimpan');
                }
            });
