...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <div>
            <a href="{{ route('admin.activities.create') }}" class="btn btn-primary btn-sm">Tambah</a>
        </div>
    </div>

    <div class="mb-2">
        <input type="text" id="search-activities" class="form-control table-search" placeholder="Cari judul atau lokasi...">
    </div>

    <div class="card"><div class="table-responsive"><table class="table">
        <thead><tr><th>Drag</th><th>Urut</th><th>Judul</th><th>Tanggal</th><th>Lokasi</th><th>Status</th><th>Aksi</th></tr></thead>
        ...
        @forelse($items as $item)
            <tr data-id="{{ $item->id }}">
                ...
                <td>{{ $item->title }}</td>
                <td>{{ optional($item->activity_date)->format('d M Y') }}</td>
                <td>{{ $item->location }}</td>
...
<script>
    (function () {
        ...
        const persistOrder = async () => {
            ...
            const response = await fetch('{{ route('admin.activities.reorder') }}', {
                method: 'POST',
                headers: {
        ...
        tbody.querySelectorAll('tr[data-id]').forEach((row) => {
            ...
            handle.addEventListener('dragend', async () => {
                ...
                if (isDirty) {
                    await persistOrder();
                    if (window.showAdminToast) window.showAdminToast('Urutan kegiatan tersimpan');
                }
            });
